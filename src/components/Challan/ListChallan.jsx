...
function ListChallan() {
  const [challan, setChallan] = useState([])
    ...
    const handleFilteration = (e) => {
        ...
        const filterItem = products.filter((item) => item.name.toLowerCase().includes(e.target.value));
        setProduct(filterItem);
    }

    const handleEdit = (id, currentName) => {
        setEditingId(id);
        setupdateProduct(currentName);
    };
    ...
    const handleUpdate = async (ChallanId) => {
        try {
            ...
            setChallan((prev) => prev.map((item) => item._id === productId ? { ...item, name: updateChallan } : item));
